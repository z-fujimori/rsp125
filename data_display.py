...
def plot_rews(rews1_timing1,rews2_timing1,rews1_timing2,rews2_timing2,result_name='output',run_time_log='--',num_trials=10000,step=5,is_save_mode=True, moving_average=True, isRobust=False):
  if (is_save_mode):
    if isRobust:
  ...
  if isRobust:
    save_plot_rews(rews1_timing1, rews2_timing1, result_name, f"{result_name}_エージェントA")
    save_plot_rews(rews1_timing2, rews2_timing2, result_name, f"{result_name}_エージェントB")
  else:
...
def display_percentage_of_hand(hist_a_timing1, hist_b_timing1, hist_a_timing2, hist_b_timing2, result_name='output',run_time_log='--', moving_averae=True, isRobust=False):
  ...
  def save_hand_hist(hist_a, hist_b, log_dir, log_name):
    ...
    # CSV保存部分
    if isRobust:
      result_name = f"./results/{log_dir}/robust/{log_name}.csv"
    else:
    ...
    if moving_averae:
      ...
      for i in range(len(hand_plot_hist[0][0])-10):
        new_hand00.append(sum(hand_plot_hist[0][0][i:i+10])/10)
        new_hand01.append(sum(hand_plot_hist[0][1][i:i+10])/10)
        new_hand02.append(sum(hand_plot_hist[0][2][i:i+10])/10)
        new_hand10.append(sum(hand_plot_hist[1][0][i:i+10])/10)
        new_hand11.append(sum(hand_plot_hist[1][1][i:i+10])/10)
        new_hand12.append(sum(hand_plot_hist[1][2][i:i+10])/10)
        new_hand20.append(sum(hand_plot_hist[2][0][i:i+10])/10)
        new_hand21.append(sum(hand_plot_hist[2][1][i:i+10])/10)
        new_hand22.append(sum(hand_plot_hist[2][2][i:i+10])/10)
...
def retaliating_plot_rews(rews1_timing1,rews2_timing1,rews1_timing2,rews2_timing2,result_name='output',run_time_log='--',num_trials=10000,step=5,is_save_mode=True, moving_average=True, isRobust=False):
  ...
  if moving_average :
    ...
    for i in range(len(rews1_timing2)-10):
      new_rews1_timing1.append(sum(rews1_timing1[i:i+10])/10)
      new_rews2_timing1.append(sum(rews2_timing1[i:i+10])/10)
      new_rews1_timing2.append(sum(rews1_timing2[i:i+10])/10)
      new_rews2_timing2.append(sum(rews2_timing2[i:i+10])/10)
